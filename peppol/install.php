...
if (!$CI->db->table_exists($peppol_docs_table)) {
    $CI->db->query('
        CREATE TABLE IF NOT EXISTS `' . db_prefix() . 'peppol_documents` (
            ...
            `provider_document_transmission_id` varchar(150) DEFAULT NULL COMMENT "Transmission ID or equivalent from notification for easy lookup",
...
if (!$CI->db->table_exists(db_prefix() . 'peppol_logs')) {
    $CI->db->query('
        CREATE TABLE IF NOT EXISTS `' . db_prefix() . 'peppol_logs` (
            `id` int(11) NOT NULL AUTO_INCREMENT,
