...
class Peppol extends AdminController
{
    ...
    public function send($invoice_id)
    {
        ...
        // Get the referrer to redirect back to current view
        $redirect_url = admin_url('invoices/list_invoices/' . $invoice_id);
        
        // If came from invoice view, redirect back there
        $referrer = $this->agent->referrer();
        if ($referrer && strpos($referrer, 'invoices/list_invoices/' . $invoice_id) !== false) {
    ...
    public function send_ajax($invoice_id)
    {
        ...
        // Check if client has PEPPOL identifier before attempting to send
